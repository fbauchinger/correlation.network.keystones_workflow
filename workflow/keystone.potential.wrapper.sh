...
SIG_CUTOFF=0.1 # p-value cutoff for observed correlations
...
for ((ID=1;ID<=ITERATIONS;ID++)); do
	...
	#mv subset_data_${ID}_log.txt $OUTPUT_PATH/log

	# rm -r bootstrap_counts
	
	# rm -r bootstrap_correlations

done

echo "calculating keystone potential"

Rscript R_scripts/calculate.keystone.potential.R $ITERATIONS $SIG_CUTOFF $USE_CLOSENESS_CENTRALITY >> "${LOGFILE}" 2>&1
...
#
# "fastspar" command not found: activate conda environment 
#
########################################################
